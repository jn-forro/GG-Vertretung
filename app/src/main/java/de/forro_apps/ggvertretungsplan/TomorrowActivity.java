...
public class TomorrowActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(ContentSelector.getDate(ContentSelector.Day.TOMORROW));

        adapter = new CustomListAdapter(ContentSelector.Day.TOMORROW, this);

        list = (ExpandableListView) findViewById(R.id.expandedList);
        ...
        if (list.getAdapter().getCount() == 0) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                noInformationFound.setText(Html.fromHtml(ContentLoader.TOMORROW_HTML, Html.FROM_HTML_MODE_LEGACY));
            } else {
                noInformationFound.setText(Html.fromHtml(ContentLoader.TOMORROW_HTML));
            }
            RelativeLayout rl = (RelativeLayout) findViewById(R.id.activity_tomorrow);
